...
WAZUH_VERSION="4.x"
WAZUH_MODE="all-in-one"  # Manager + Indexer + Dashboard
MIN_RAM=4
MIN_DISK=60
...
show_banner() {
    ...
    echo "  ║       Installation Snort + Wazuh All-in-One         ║"
...
# ================================================================
install_wazuh() {
    log_etape "4/7" "INSTALLATION WAZUH ALL-IN-ONE (Manager + Indexer + Dashboard)"
    log_info "Composants : Manager + Indexer + Dashboard"
    log_warn "Cette etape peut prendre 30 a 60 minutes selon votre connexion."
    echo ""
    ...
    bash wazuh-install.sh -a 2>&1  # -a = all-in-one | tee -a "$LOG_FILE" | \
...
── WAZUH MANAGER ─────────────────────────────────────────────
  ...
  Dashboard   : https://${SERVER_IP}  (port 443)
